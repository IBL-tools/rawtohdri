...
(defun stack-images (layers ev-step lo hi center)
  "Stacks multiple exposure layers component-wise and returns the combined image (mutates the first layer in-place)."
  ...
  (let* ((bg (first layers))
         (fgs (rest layers))
         (size (length bg)))
    (declare (type (simple-array single-float (*)) bg)
             (type fixnum size))
